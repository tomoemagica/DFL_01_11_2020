...
class Model(ModelBase):
    ...
    def onGetPreview(self, sample):
        ...
        return [ ('LIAEF256', np.concatenate ( st, axis=0 ) ) ]

    def predictor_func (self, face=None, dummy_predict=False):
        if dummy_predict:
            self.convert ([ np.zeros ( (1, 256, 256, 3), dtype=np.float32 ) ])
        else:
    ...
    def get_ConverterConfig(self):
        import converters
        return self.predictor_func, (256,256,3), converters.ConverterConfigMasked(face_type=FaceType.FULL, default_mode='seamless')
    ...
    def Build(self, input_layer):
        ...
        def Intermediate():
            input_layer = Input(shape=(None, 16 * 16 * 1024))
            x = input_layer
            x = Dense(256)(x)
            x = Dense(16 * 16 * 256)(x)
